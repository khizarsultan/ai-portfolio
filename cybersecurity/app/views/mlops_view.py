...
st.title("Malicious URL MLOps Pipeline")
...
with t_reg:
    ...
    if versions:
        ...
        st.dataframe(df.rename(columns={"version": "ver", "roc_auc": "Macro-F1", "pr_auc": "Macro-recall"}),
...
with t_run:
    ...
    if g[2].button("3 · Retrain", use_container_width=True):
        ...
        (st.error(err) if err else st.success(f"Challenger v{rec['version']} registered "
                                              f"(macro-F1 {rec['metrics']['macro_f1']:.4f})."))
...
with t_cmp:
    ...
    if not rec:
        ...
        keys = ["macro_f1", "macro_recall", "accuracy"]
        fig = go.Figure()
