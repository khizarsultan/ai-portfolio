"use client";
// MLOps Pipeline (URL). Registry + drift monitor are REAL (mlflow.db + live PSI).
// The champion-vs-challenger + approval cycle is a clearly-badged illustrative
// scenario — there is no committed challenger state and the raw data is gone, so
      ...
      {STEPS.map((s, i) => {
        ...
        return (
          <g key={i}>
            ...
            {i < n - 1 && <line x1={x + bw} y1={35} x2={x + bw + gap} y2={35} stroke={C.muted} strokeWidth={1.5} markerEnd="url(#aru)" />}
          </g>
        );
      })}
      <defs><marker id="aru" markerWidth="7" markerHeight="7" refX="6" refY="3" orient="auto"><path d="M0,0 L6,3 L0,6 Z" fill={C.muted} /></marker></defs>
    </svg>
  );
}
  ...
  return (
    <div className="space-y-5">
      ...
      <Card className="border-amber-200 dark:border-amber-900/60">
        ...
        <p className="mb-3 text-xs text-slate-400">Illustrative: the current champion&apos;s metrics are real; the challenger v{m.challenger.version} is a hypothetical retrain to demonstrate the promotion gate.</p>
        <GroupedBars groups={["Macro-F1", "Macro-recall", "ROC-AUC", "Accuracy"]}
          series={[
